...
import 'package:tatsam/Screens/dashboard/bloc/dashboard_event.dart';
...
class BusinessFormEditScreen extends StatefulWidget {
  const BusinessFormEditScreen({Key? key}) : super(key: key);

  @override
  State<BusinessFormEditScreen> createState() => _BusinessFormEditScreenState();
}

class _BusinessFormEditScreenState extends State<BusinessFormEditScreen> {
  BusinessFormEditBloc businessFormEditBloc = BusinessFormEditBloc();
  ...
  void initState() {
    super.initState();
    businessData = context.read<DashboardBloc>().editBusinessData;
    imageURL = businessData.businessImagePath ?? '';
    businessLogoImage = File('');
    _imagePicker = ImagePicker();
    dropdownvalue = Strings.architecture;
    businessTypeId = StringDigits.one;
    setBusinessType();
    nameController = TextEditingController(text: businessData.user!.name ?? '');
    workNameController =
        TextEditingController(text: businessData.businessName ?? '');
    mobileNumberController =
        TextEditingController(text: businessData.businessPhoneNo ?? '');
    scaffoldState = GlobalKey<ScaffoldState>();
  }

  setBusinessType() {
    switch (businessData.businessType!.typeName) {
      case Strings.architecture:
        dropdownvalue = Strings.architecture;
        businessTypeId = StringDigits.one;
        break;
      case Strings.doctor:
        dropdownvalue = Strings.doctor;
        businessTypeId = StringDigits.two;
        break;
      case Strings.plumber:
        dropdownvalue = Strings.plumber;
        businessTypeId = StringDigits.three;
        break;
      case Strings.ui:
        dropdownvalue = Strings.ui;
  ...
  _getImage(ImageSource source) async {
    ...
    if (pickedFile.path.isNotEmpty) {
      businessFormEditBloc.add(
        BusinessImageChangeEvent(pickedImage: File(pickedFile.path)),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return SafeArea(
      child: Scaffold(
        ...
        body: BlocConsumer(
          bloc: businessFormEditBloc,
          listener: (context, state) {
            if (state is BusinessFormEditLoadingStartState) {
              isLoading = true;
            }
            if (state is BusinessFormEditLoadingEndState) {
              isLoading = false;
            }
            if (state is BusinessTypeChangeState) {
            ...
            if (state is BusinessImageChangeState) {
              businessLogoImage = state.pickedImage;
            }
            if (state is UpdateBusinessState) {
            ...
            if (state is BusinessFormEditErrorState) {
